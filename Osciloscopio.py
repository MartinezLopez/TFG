...
class Osciloscopio:
  ...
  def __init__(self, id):
    ...
    Aqui se crea un objeto de la clase Osciloscopio y se inicializan los diccionarios 
    que empleara para asegurarse de que los valores de las configuraciones son valores 
    aceptados por el osciloscopio Tektronix 2022B.
    ...
    Parametros:
      id: identificador valido del osciloscopio (string de la forma 0xYYYY::0xZZZZ donde YYYY 
        es el idVendor y ZZZZ el idProduct del osciloscopio).
    '''

    # Inicializamos los diccionarios para que tengan los valores validos para el osciloscopio
    self.vol_div = {"5v":'5', "2v":'2', "1v":'1', "500mv":'500e-3', "200mv":'200e-3', "100mv":'100e-3', 
      "50mv":'50e-3', "20mv":'20e-3', "10mv":'10e-3', "5mv":'5e-3', "2mv":'2e-3'}
    self.sec_div = {"50s":'50', "25s":'25', "10s":'10', "5s":'5', "2.5s":'2.5', "1s":'1',"500ms":'500e-3', 
      "250ms":'250e-3', "100ms":'100e-3',"50ms":'50e-3', "25ms":'25e-3', "10ms":'10e-3', "5ms":'5e-3', 
      "2.5ms":'2.5e-3', "1ms":'1e-3', "500us":'500e-6', "250us":'250e-6', "100us":'100e-6',"50us":'50e-6', 
      "25us":'25e-6', "10us":'10e-6', "5us":'5e-6', "2.5us":'2.5e-6', "1us":'1e-6', "500ns":'500e-9', 
      "250ns":'250e-9', "100ns":'100e-9',"50ns":'50e-9', "25ns":'25e-9', "10ns":'10e-9', "5ns":'5e-9', "2.5ns":'2.5e-9'}
    self.acoplamiento = {"AC":'AC', "DC":'DC', "GND":'GND'}
    self.canal = {"1":'CH1', "2":'CH2'}
    self.atenuacion = {"1":'1', "10":'10'} 
    self.bytes_medida = {"2":'2', "1":'1'}
    self.medidas = {"frecuencia":'FREQ', "periodo":'PERI', "vmedio":'MEAN', "vpp":'PK2', "vrms":'CRM', "vmin":'MINI', 
      "vmax":'MAXI', "tsubida":'RIS', "tbajada":'FALL'}
  ...
  def set_display(self, mode):
    ...
    if mode == "XY":  self.ins.write("DIS:FORM XY")
    else:             self.ins.write("DIS:FORM YT")
  ...
  def set_trigger(self, channel, level):
    ...
    self.ins.write("TRIG:MAI:LEV %i;EDGE:SOU %s" %(level,self.canal_trigg[channel]))
  ...
  def set_vertical(self, channel, v_d, coupling, probe):
    ...
    self.ins.write("%s:COUP %s;PRO %s;VOL %s"  %(self.canal[channel], self.acoplamiento[coupling], probe, self.vol_div[v_d]))
  ...
  def get_data(self, source, start, stop, width):
    ...
    intentos = 0
    while intentos < 8:
      try: # Es un manejo de la excepcion un poco largo, pero al haber varias llamadas parecidas puede fallar cualquiera 
           # y hay veces que no te das cuenta del fallo hasta el procesado de datos que hace al final
        codificacion = "RIB" #Entre 127 y -128 con un byte
        ch = self.canal[source]
        start = 1 if start < 1
        stop = 2500 if stop > 2500
        prec = self.bytes_medida[width]
        self.ins.write("DAT:ENC %s;SOU %s;STAR %i;STOP %i;WID "  %(codificacion, ch, start, stop, prec))
        ...
        #Si la resolucion no es dos la consideramos uno; #32767/5; #127/5
        escala = 6553.4 if prec == '2' else 25.4
        tension =[dato*v_div/escala for dat in puntos]
  ...
  def disp_channel(self, state, channel):
    ...
    if(state == True):  self.ins.write("SEL:%s ON" %ch)
    else:               self.ins.write("SEL:%s OFF" %ch)
  ...
  def get_measure(self, channel, medida):
    ...
    # Los sleep despues de cada escritura son necesarios porque al osciloscopio no le da tiempo a 
    # configurarse tan rapido y devuelve un error
    ch = self.canal[channel]
  ...
  def formatter(self, value):
    ...
    Dependiendo del exponente asigna una letra para la magnitud de las unidades y multiplica 
    por un valor de 1, 10 o 100 segun corresponda.
    ...
    new_exp = exp
    mult=10**int(exp.partition('-')[2])%3

    if      exp.partition('-')[2]<=-10: new_exp = 'p'
    else if exp.partition('-')[2]<= -7: new_exp = 'n'
    else if exp.partition('-')[2]<= -4: new_exp = 'u'
    else if exp.partition('-')[2]<= -1: new_exp = 'm'
    else if exp.partition('-')[2]<=  2: new_exp = ''
    else if exp.partition('-')[2]<=  5: new_exp = 'k'
    else if exp.partition('-')[2]<=  8: new_exp = 'M'
    else if exp.partition('-')[2]<= 11: new_exp = 'G'
    else if exp.partition('-')[2]<= 12: new_exp = 'T'
    
    num = str(number * mult)
    dot = num.find('.')
    num = num[0:(dot+prec)]
    num = "%s %s" %(num, new_exp)
    num = 'Err ' if num == '9.9 E37'
    return num
